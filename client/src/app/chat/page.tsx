...
const router = useRouter();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Uses Axios under the hood now
      const response = await chatService.sendMessage(userText);

      const aiMessage: ChatApiResponse = {
  ...
  return (
    <ProtectedRoute>
      <div className="flex h-screen bg-[#1F242E] text-[#F3F4F6] overflow-hidden font-sans">
        ...
        <aside
          className={`fixed md:relative z-30 w-64 h-full bg-[#181C24] border-r border-gray-800 flex flex-col transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
            }`}
        >
          ...
          <button onClick={handleLogout} className="bg-red-400 py-2 rounded-2xl hover:bg-red-500 cursor-pointer transition-all">
            Logout
          </button>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
            {messages.map((msg) => (
              ...
              >
                {/* Avatar */}
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-white ${msg.sender === 'user' ? 'bg-[#3B82F6]' : 'bg-[#181C24] border border-gray-700'
                    }`}
                >
                  {msg.sender === 'user' ? (
                ...
                <div className="flex flex-col space-y-1 max-w-[85%] md:max-w-[75%]">
                  ...
                  <span
                    className={`text-[10px] text-gray-500 ${msg.sender === 'user' ? 'text-right' : 'text-left'
                      }`}
                  >
                    {msg.timestamp}
                  </span>
